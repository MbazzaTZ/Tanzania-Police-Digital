            ...
            <div style={{height:180,display:'flex',alignItems:'flex-end',gap:6}}>
              {monthly.map(m=>(
                <div key={m.m} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:3}}>
                  <div style={{fontSize:8,color:'var(--clr-accent)',fontFamily:'var(--font-mono)'}}>{m.v||''}</div>
                  <div style={{width:'100%',background:m.v?'var(--clr-primary)':'var(--clr-border)',height:`${m.v?(m.v/max)*140:4}px`,borderRadius:'3px 3px 0 0',transition:'height .3s'}} />
                  <div style={{fontSize:9,color:'var(--clr-muted)'}}>{m.m}</div>
                </div>
              ))}
            </div>
          </CardBody>
            ...
            {MOCK_REGIONS_STATS.map(r=>(
              <div key={r.name} style={{marginBottom:10}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}>
                  <span style={{fontSize:11,color:'var(--clr-text)'}}>{r.name}</span>
                  <span style={{fontSize:10,fontFamily:'var(--font-mono)',color:'var(--clr-accent)'}}>{r.count}</span>
                </div>
                <div style={{background:'var(--clr-dark)',borderRadius:4,height:6}}>
        ...
        <CardBody>
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
            {[['Wizi','1,234','#EF5350'],['Mapigano','567','#FF7043'],['Madawa','445','#7E57C2'],['Udanganyifu','334','#42A5F5'],['Mauaji','89','#EF5350'],['Ubakaji','112','#EF5350'],['Ugaidi','12','#B71C1C'],['Uhalifu wa Mtandaoni','145','#7E57C2']].map(([type,count,color])=>(
              <div key={type} style={{background:'var(--clr-dark)',borderRadius:'var(--r)',padding:'12px 14px',borderLeft:`3px solid ${color}`}}>
                <div style={{fontSize:11,color:'var(--clr-muted)',marginBottom:4}}>{type}</div>
                <div style={{fontSize:20,fontWeight:800,fontFamily:'var(--font-mono)',color:'var(--clr-white)'}}>{count}</div>
              </div>
            ))}
          </div>
        </CardBody>
