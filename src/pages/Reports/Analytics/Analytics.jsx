  ...
  return (
    <div className="afd">
      ...
      <div className="ph">
        ...
        <div className="flex g8"><Button variant="o">⬇ Pakua</Button><Button variant="g">🤖 AI Ripoti</Button></div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <Card>
          ...
          <CardBody>
            {[['Kariakoo, Dar es Salaam','Wizi wa Mkoba',92,'#EF5350'],['Temeke, Dar es Salaam','Mapigano',87,'#FF7043'],['Mwanza CBD','Ulaghai',78,'#FF7043'],['Arusha Town','Wizi wa Gari',71,'var(--gold)'],['Dodoma City','Madawa',65,'var(--gold)']].map(([loc,type,pct,color])=>(
              <div key={loc} style={{padding:'10px 0',borderBottom:'1px solid rgba(255,255,255,.04)'}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                  <div><div style={{fontSize:12,fontWeight:600,color:'var(--text-white)'}}>{loc}</div><div style={{fontSize:10,color:'var(--text-muted)'}}>{type}</div></div>
                  <div style={{fontSize:14,fontWeight:800,fontFamily:'var(--font-mono)',color}}>{pct}%</div>
                </div>
                <div style={{background:'var(--green-900)',borderRadius:4,height:5}}><div style={{height:'100%',background:color,borderRadius:4,width:`${pct}%`}} /></div>
              </div>
            ))}
          </CardBody>
        </Card>
        <Card>
          ...
          <CardBody>
            <div className="chart-placeholder" style={{height:180,marginBottom:12}}>
              ...
              <div style={{fontSize:10,color:'var(--text-muted)'}}>Recharts itaunganishwa Sprint 2</div>
            </div>
            {[['00:00–06:00','Usiku wa manane','25%','#7E57C2'],['06:00–12:00','Asubuhi','18%','#42A5F5'],['12:00–18:00','Mchana','22%','var(--gold)'],['18:00–24:00','Jioni','35%','#EF5350']].map(([time,label,pct,color])=>(
