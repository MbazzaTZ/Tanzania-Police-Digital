      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 2fr',gap:16,height:500}}>
          ...
          <div style={{flex:1,overflowY:'auto',padding:8}}>
            {MOCK_MESSAGES.map(m => (
              <div key={m.id} onClick={() => setSelected(m)}
                style={{padding:'10px 12px',borderRadius:'var(--r)',marginBottom:4,cursor:'pointer',background:selected?.id===m.id?'var(--green-600)':'transparent',border:'1px solid',borderColor:selected?.id===m.id?'var(--green-500)':'var(--border)'}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                  <div style={{fontSize:12,fontWeight:m.read?500:700,color:'var(--text-white)'}}>{m.from}</div>
                ...
                <div style={{fontSize:11,color:'var(--text-muted)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{m.content}</div>
                <div style={{fontSize:9,color:'var(--text-muted)',marginTop:4,opacity:.7}}>{m.time}</div>
              </div>
            ))}
          </div>
        </Card>
        <Card style={{display:'flex',flexDirection:'column'}}>
          {selected ? (
            ...
            <CardBody><div style={{textAlign:'center',padding:40,color:'var(--text-muted)'}}>Chagua ujumbe kuona</div></CardBody>
          )}
        </Card>
      </div>
    </div>
