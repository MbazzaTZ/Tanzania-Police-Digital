    ...
    <div className="afd">
      ...
      <Card>
        <CardHeader title="🗂️ Vitendo vya Hivi Karibuni"
          action={<select className="fs" style={{width:'auto',fontSize:11,padding:'6px 10px'}}><option>Vitendo Vyote</option>{AUDIT_ACTIONS.map(a => <option key={a}>{a}</option>)}</select>} />
        <CardBody noPadding>
          <div className="tw">
            <table>
              ...
              <tbody>
                {MOCK_AUDIT.map(a => (
                  <tr key={a.id}>
                    ...
                    <td><div className="td-nm">{a.officer}</div><div className="td-sub">Badge: {a.badge}</div></td>
                    ...
                    <td className="td-mo">{a.record}</td>
