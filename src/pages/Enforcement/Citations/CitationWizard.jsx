...
export default function CitationWizard() {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ maxWidth:860, margin:'0 auto', background:'var(--clr-panel)', border:'1px solid var(--clr-border)', borderRadius:'var(--r-lg)' }}>
        ...
        {step === 1 && (
          <div>
            ...
            <div className="card-body">
              <div style={{ fontSize:11, fontWeight:700, color:'var(--clr-accent)', textTransform:'uppercase', letterSpacing:1, marginBottom:12, paddingBottom:8, borderBottom:'1px solid var(--clr-border)' }}>🔍 Tafuta Mtuhumiwa</div>
              ...
              {/* Person found */}
              <div style={{ background:'var(--clr-dark)', border:'1px solid var(--clr-secondary)', borderRadius:'var(--r)', padding:'12px 14px', display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
                <div style={{ width:48, height:48, borderRadius:'50%', background:'var(--clr-primary)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22 }}>👤</div>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:14, fontWeight:700, color:'#fff' }}>{p.fullName}</div>
                  <div style={{ fontSize:11, color:'var(--clr-muted)', marginTop:2 }}>NIDA: {p.nida} · DOB: {p.dob} ({p.age} yrs) · {p.gender}</div>
                  <div style={{ fontSize:11, color:'var(--clr-muted)' }}>{p.phone} · {p.address}, {p.district}</div>
                </div>
              ...
              <div style={{ fontSize:11, fontWeight:700, color:'var(--clr-accent)', textTransform:'uppercase', letterSpacing:1, marginBottom:12, paddingBottom:8, borderBottom:'1px solid var(--clr-border)' }}>🚗 Taarifa za Gari</div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:16 }}>
                {TRAFFIC_OFFENCES.map(o => (
                  <div key={o.code} onClick={() => setOffence(o)}
                    style={{ border:`1px solid ${offence?.code === o.code ? 'var(--clr-accent)' : 'var(--clr-border)'}`, background: offence?.code === o.code ? 'rgba(255,193,7,.06)' : 'transparent', borderRadius:'var(--r)', padding:'10px 12px', cursor:'pointer', transition:'all .15s' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6 }}>
                      <div style={{ width:14, height:14, borderRadius:'50%', border:'2px solid', borderColor: offence?.code === o.code ? 'var(--clr-accent)' : 'var(--clr-border)', background: offence?.code === o.code ? 'var(--clr-accent)' : 'transparent', flexShrink:0, transition:'all .15s' }} />
                      <div style={{ fontFamily:'var(--font-mono)', fontSize:10, color:'var(--clr-accent)' }}>Kif. {o.code}</div>
                    </div>
                    <div style={{ fontSize:12, fontWeight:600, color:'var(--clr-white)', marginBottom:2 }}>{o.name}</div>
                    <div style={{ fontSize:10, color:'var(--clr-muted)' }}>{o.law}</div>
        ...
        {step === 3 && (
          <div>
            ...
            <div className="card-body">
              <div className="form-grid-2" style={{ marginBottom:16 }}>
                <div className="form-group"><label className="form-label">Namba ya Citation</label><input className="form-input" readOnly value="CIT-2024-00029" style={{ color:'var(--clr-accent)', fontFamily:'var(--font-mono)' }} /></div>
              ...
              <div style={{ fontSize:11, fontWeight:700, color:'var(--clr-accent)', textTransform:'uppercase', marginBottom:8 }}>📷 Picha na Ushahidi</div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, marginBottom:16 }}>
                {['🚗','📋'].map((e,i) => (
                  <div key={i} style={{ background:'var(--clr-dark)', borderRadius:'var(--r)', aspectRatio:1, display:'flex', alignItems:'center', justifyContent:'center', fontSize:28 }}>{e}</div>
                ))}
                {['📷','🎥'].map((e,i) => (
                  <div key={i} style={{ border:'2px dashed var(--clr-border)', borderRadius:'var(--r)', aspectRatio:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:4, cursor:'pointer' }}>
                    <div style={{ fontSize:24 }}>{e}</div>
                    <div style={{ fontSize:10, color:'var(--clr-muted)' }}>Ongeza</div>
                  </div>
                ))}
              </div>
              <div className="form-group"><label className="form-label">Saini ya Afisa *</label>
                <div style={{ background:'var(--clr-dark)', border:'1px solid var(--clr-border)', borderRadius:'var(--r)', height:80, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--clr-muted)', fontSize:12, cursor:'pointer' }}>
          ...
          <div>
            ...
            <div className="card-body">
              {[
                ...
                <div key={section.title} style={{ background:'var(--clr-dark)', borderRadius:'var(--r)', padding:'12px 14px', marginBottom:12 }}>
                  <div style={{ fontSize:11, fontWeight:700, color:'var(--clr-accent)', marginBottom:8 }}>{section.title}</div>
                  {section.rows.map(([label, val]) => (
                    <div key={label} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', fontSize:12, borderBottom:'1px solid rgba(255,255,255,.04)' }}>
                      ...
                      <span style={{ color:'var(--clr-white)', fontWeight:500 }}>{val}</span>
                    </div>
                  ))}
                </div>
              ))}
        ...
        {/* Nav */}
        <div style={{ padding:16, borderTop:'1px solid var(--clr-border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          {step > 1
            ? <Button variant="outline" onClick={() => setStep(s => s - 1)}>← Nyuma</Button>
            : <div />}
          <div style={{ fontSize:11, color:'var(--clr-muted)' }}>Hatua ya {step} ya {STEPS.length}: {STEPS[step-1]}</div>
          {step < STEPS.length
